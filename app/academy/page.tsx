// v1 de-risked — educational reference only
// No advice, certification, or compliance guidance
// Do not expand scope without intent review

import Link from "next/link";

import { Section, Grid } from "@/components/layout/Section";
import { Card, CardHeader } from "@/components/ui/Card";
import { StatusBadge } from "@/components/ui/StatusBadge";
import { LinkButton } from "@/components/ui/Button";
import { MetricTile } from "@/components/ui/MetricTile";

import { ACADEMY_COURSES, type Course } from "@/lib/academy/courses";

function groupByLevel(courses: Course[]) {
  const buckets: Record<Course["level"], Course[]> = { L1: [], L2: [], L3: [], L4: [] };
  for (const c of courses) buckets[c.level].push(c);
  return buckets;
}

function levelLabel(level: Course["level"]) {
  switch (level) {
    case "L1":
      return "FOUNDATIONS";
    case "L2":
      return "EVIDENCE OPS";
    case "L3":
      return "DISCLOSURE";
    case "L4":
      return "ENTERPRISE";
  }
}

function pricingLabel(level: Course["level"]) {
  if (level === "L1") return "Free / Low-cost";
  if (level === "L2") return "Paid";
  if (level === "L3") return "Premium";
  return "Custom";
}

function pickFeatured(courses: Course[]) {
  const preferredIds = ["evidence-operations", "evidence-not-outcomes", "esg-foundations"];
  for (const id of preferredIds) {
    const hit = courses.find((c) => c.id === id);
    if (hit) return hit;
  }
  return courses[0] ?? null;
}

function pickRecommendedNext(
  grouped: Record<Course["level"], Course[]>,
  featured: Course | null
) {
  if (!featured) return null;
  if (featured.level === "L1") return grouped.L2[0] ?? grouped.L3[0] ?? null;
  if (featured.level === "L2") return grouped.L3[0] ?? null;
  if (featured.level === "L3") return grouped.L4[0] ?? null;
  return null;
}

function TrackCard({
  title,
  subtitle,
  courses,
  cta,
}: {
  title: string;
  subtitle: string;
  courses: Course[];
  cta: { href: string; label: string };
}) {
  return (
    <Card>
      <CardHeader
        title={title}
        subtitle={subtitle}
        right={<StatusBadge tone="neutral" mono>TRACK</StatusBadge>}
      />

      <ul className="mt-3 list-disc pl-5 text-sm text-text-300 space-y-1">
        {courses.slice(0, 4).map((c) => (
          <li key={c.id}>
            <Link
              href={`/academy/courses/${c.id}`}
              className="text-text-200 hover:underline underline-offset-4"
            >
              {c.title}
            </Link>
          </li>
        ))}
      </ul>

      {courses.length > 4 ? (
        <div className="mt-2 text-xs text-text-400">+ {courses.length - 4} more</div>
      ) : null}

      <div className="mt-4 grid grid-cols-2 gap-3">
        <MetricTile label="Courses" value={String(courses.length)} note="available" mono />
        <MetricTile
          label="Pricing"
          value={courses[0] ? pricingLabel(courses[0].level) : "—"}
          note="tier"
          mono
        />
      </div>

      <div className="mt-4 flex justify-end">
        <LinkButton href={cta.href} variant="secondary">
          {cta.label}
        </LinkButton>
      </div>

      <div className="mt-3 text-xs text-text-400 leading-relaxed">
        Educational material only. No regulatory accreditation, certification, or compliance
        determination.
      </div>
    </Card>
  );
}

function CourseMiniCard({
  title,
  subtitle,
  badgeLeft,
  badgeRight,
  href,
  bullets,
}: {
  title: string;
  subtitle: string;
  badgeLeft: string;
  badgeRight: string;
  href: string;
  bullets: string[];
}) {
  return (
    <Card>
      <CardHeader
        title={title}
        subtitle={subtitle}
        right={
          <div className="flex items-center gap-2">
            <StatusBadge tone="neutral" mono>{badgeLeft}</StatusBadge>
            <StatusBadge tone="neutral" mono>{badgeRight}</StatusBadge>
          </div>
        }
      />
      <ul className="mt-2 list-disc pl-5 text-sm text-text-300 space-y-1">
        {bullets.slice(0, 3).map((b) => (
          <li key={b}>{b}</li>
        ))}
      </ul>
      <div className="mt-4 flex items-center justify-between gap-3">
        <Link
          href={href}
          className="text-sm text-accent-400 hover:text-accent-500 transition-colors"
        >
          View syllabus →
        </Link>
        <LinkButton href="/contact?topic=academy" variant="secondary">
          Education inquiry
        </LinkButton>
      </div>
      <div className="mt-3 text-xs text-text-400 leading-relaxed">
        Completion confirms learning only. No compliance or regulatory outcomes are implied.
      </div>
    </Card>
  );
}

export default function AcademyPage() {
  const grouped = groupByLevel(ACADEMY_COURSES);
  const featured = pickFeatured(ACADEMY_COURSES);
  const recommendedNext = pickRecommendedNext(grouped, featured);

  return (
    <>
      {/* Hero */}
      <Section>
        <Grid>
          <div className="md:col-span-8">
            <div className="flex items-center gap-2">
              <StatusBadge tone="neutral" mono>ACADEMY</StatusBadge>
              <StatusBadge tone="neutral" mono>REFERENCE</StatusBadge>
            </div>

            <h1 className="mt-4 text-3xl font-semibold">EcoVeraZ Academy</h1>

            <p className="mt-3 text-lg text-text-200">
              Educational reference material for understanding ESG evidence and review contexts.
            </p>

            <p className="mt-3 text-sm text-text-300 leading-relaxed">
              EcoVeraZ Academy provides conceptual orientation to how environmental evidence is
              structured, reviewed, and disclosed. Content supports shared vocabulary and literacy
              without prescribing actions, outcomes, or interpretations.
            </p>

            <div className="mt-6 flex flex-wrap gap-2">
              <LinkButton href="/academy/courses" variant="primary">
                View reference material
              </LinkButton>
              <LinkButton href="/resources" variant="secondary">
                Evaluator resources
              </LinkButton>
            </div>

            <div className="mt-4 text-xs text-text-400">
              Canonical references:{" "}
              <Link href="/compliance" className="underline underline-offset-4">
                Operating principles
              </Link>
              {" · "}
              <Link href="/ladder" className="underline underline-offset-4">
                Evidence Ladder
              </Link>
              {" · "}
              <Link href="/rri" className="underline underline-offset-4">
                RRI
              </Link>
            </div>
          </div>
        </Grid>
      </Section>

      {/* Featured + Next */}
      <Section
        title="Start here"
        subtitle="A neutral entry point: one reference module, followed by a commonly adjacent topic."
      >
        <Grid>
          <div className="md:col-span-12 grid gap-6 md:grid-cols-2">
            {featured ? (
              <CourseMiniCard
                title={featured.title}
                subtitle={featured.subtitle}
                badgeLeft="REFERENCE"
                badgeRight={levelLabel(featured.level)}
                href={`/academy/courses/${featured.id}`}
                bullets={[
                  `Audience: ${featured.audience}`,
                  `Duration: ${featured.durationHours}`,
                  `Certificate: ${featured.certificate}`,
                ]}
              />
            ) : (
              <Card>
                <CardHeader
                  title="Reference module"
                  subtitle="No courses configured yet."
                  right={<StatusBadge tone="neutral" mono>EMPTY</StatusBadge>}
                />
                <div className="text-sm text-text-300">
                  Add courses in <span className="text-text-200">lib/academy/courses.ts</span>.
                </div>
              </Card>
            )}

            {recommendedNext ? (
              <CourseMiniCard
                title={recommendedNext.title}
                subtitle="Commonly adjacent topic"
                badgeLeft="NEXT"
                badgeRight={levelLabel(recommendedNext.level)}
                href={`/academy/courses/${recommendedNext.id}`}
                bullets={[
                  `Observed sequence: ${levelLabel(featured!.level)} → ${levelLabel(recommendedNext.level)}`,
                  `Duration: ${recommendedNext.durationHours}`,
                  `Access: ${pricingLabel(recommendedNext.level)}`,
                ]}
              />
            ) : (
              <Card>
                <CardHeader
                  title="Adjacent topics"
                  subtitle="Additional material may be relevant depending on review context."
                  right={<StatusBadge tone="neutral" mono>NEXT</StatusBadge>}
                />
                <div className="text-sm text-text-300 leading-relaxed">
                  For teams evaluating reference material at scale, EcoVeraZ can suggest content
                  groupings based on disclosure cycle and evidence scope.
                </div>
                <div className="mt-4 flex justify-end">
                  <LinkButton href="/contact?topic=academy" variant="secondary">
                    Education inquiry
                  </LinkButton>
                </div>
              </Card>
            )}
          </div>

          <div className="md:col-span-12">
            <div className="mt-6 rounded-xl border border-border bg-surface-2 px-4 py-3">
              <div className="text-xs font-medium text-text-400">Certificate boundary</div>
              <div className="mt-1 text-xs text-text-400 leading-relaxed">
                Certificates confirm completion of reference material only. They do not certify
                regulatory compliance, audit approval, or sustainability performance.
              </div>
            </div>
          </div>
        </Grid>
      </Section>

      {/* Tracks */}
      <Section
        title="Reference tracks"
        subtitle="Conceptual groupings used to organize material — not prescribed sequences."
      >
        <Grid>
          <div className="md:col-span-12 grid gap-6 md:grid-cols-3">
            <TrackCard
              title="Foundations"
              subtitle="Terminology, scope boundaries, and mental models."
              courses={grouped.L1}
              cta={{ href: "/academy/courses", label: "View foundations" }}
            />
            <TrackCard
              title="Evidence operations"
              subtitle="How review-ready evidence is commonly structured."
              courses={grouped.L2}
              cta={{ href: "/academy/courses", label: "View evidence topics" }}
            />
            <TrackCard
              title="Disclosure workflows"
              subtitle="How evidence supports reporting and review."
              courses={grouped.L3}
              cta={{ href: "/academy/courses", label: "View disclosure topics" }}
            />
          </div>
        </Grid>
      </Section>

      {/* Fit */}
      <Section
        title="How Academy fits EcoVeraZ"
        subtitle="Shared literacy supports adoption without outcome claims."
      >
        <Grid>
          <div className="md:col-span-12">
            <Card>
              <CardHeader
                title="Academy → Platform"
                subtitle="Build shared evidence literacy before deploying infrastructure."
                right={<StatusBadge tone="neutral" mono>SAFE</StatusBadge>}
              />
              <div className="text-sm text-text-300 leading-relaxed">
                Academy material supports teams in understanding evidence concepts and reviewer
                expectations. EcoVeraZ platform deployment then provides artifacts, posture signals,
                and verification surfaces used during external review.
              </div>
            </Card>
          </div>
        </Grid>
      </Section>
    </>
  );
}
